...
############################################# BHM_IG #################################################
### find c_eff
load('Rdata/tableS4/bhm_ig_1.Rdata')
...
for(i in c(1,10,12,37,39,28,30,31,34)){
  load(paste0('Rdata/tableS4/bhm_ig_',i,'.Rdata'))
  trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
...
############################################# CBHM #################################################
### find c_eff
load('Rdata/tableS4/cbhm_1.Rdata')
...
num=1
for(i in c(1,10,12,37,39,28,30,31,34)){
  load(paste0('Rdata/tableS4/cbhm_',i,'.Rdata'))
  trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
  final_eff<-t(sapply(1:Ngroup,function(r) pr_eff[r,]>c_eff[r]))
  final_eff<-final_eff*1*trial_final
  power<-rowMeans(final_eff)
...
############################################# CBHM_Tox #################################################
### find c_eff
load('Rdata/tableS4/cbhmtox_1.Rdata')
trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
c_eff<-sapply(1:Ngroup,function(r) quantile(pr_eff[r,]*trial_final[r,],.9))
...
num=1
for(i in c(1,10,12,37,39,28,30,31,34)){
  load(paste0('Rdata/tableS4/cbhmtox_',i,'.Rdata'))
  trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
  final_eff<-t(sapply(1:Ngroup,function(r) pr_eff[r,]>c_eff[r]))
  final_eff<-final_eff*1*trial_final
  power<-rowMeans(final_eff)
...
############################################# GCBHM_l #################################################
### find c_eff
load('Rdata/tableS4/gcbhm_1.Rdata')
trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
c_eff<-sapply(1:Ngroup,function(r) quantile(pr_eff[r,]*trial_final[r,],.9))
...
num=1
for(i in c(1,10,12,37,39,28,30,31,34)){
  load(paste0('Rdata/tableS4/gcbhm_',i,'.Rdata'))
  trial_final<-matrix(1, Ngroup,nsimu)*(n==(ngroup0[1]*nstage))
  final_eff<-t(sapply(1:Ngroup,function(r) pr_eff[r,]>c_eff[r]))
  final_eff<-final_eff*1*trial_final
  power<-rowMeans(final_eff)
...
write.csv(sum_res,file = "Results/Table_S4.csv")
