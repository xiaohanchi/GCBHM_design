...
p<-p[,,14] #change scenarios by setting the index here
q1<-c(0.25,0.5)
q0<-c(0.15,0.3)
...
##save data
save(Pr_futility,pr_eff,n,p_hat,file="Rdata/bhm_14.Rdata")
...
print(formatC(matrix(p_hat_result,Ncategory,Ngroup),digits=2, format="f"),quote = F) 
print("estimated shrinkage parameter at each interim analysis")
cat(formatC(c(sigma2_hat), digits=2, format="f"), sep ="  ", "\n") 
