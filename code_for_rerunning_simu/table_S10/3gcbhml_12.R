...
trans_y<-function(t,tp_expand,Ngroup){
  Y_1<-1*(multi_Y0==1)[1:(ngroup_all[t,1]),,]*tp_expand
  Y_2<-1*(multi_Y0==2)[1:(ngroup_all[t,1]),,]*tp_expand
  Y_3<-1*(multi_Y0==3)[1:(ngroup_all[t,1]),,]*tp_expand
  Y_4<-1*(multi_Y0==4)[1:(ngroup_all[t,1]),,]*tp_expand
...
ninterim<-c(12,12,12,9)#interim sample size
...
multi_Y0<-sapply(1:nsimu,function(m) sapply(1:Ngroup, function(l) sapply(1:ngroup_all[nstage,1], function(r) which(rmultinom(ngroup_all[nstage,1],1,p[,l])[,r]==1))),simplify = 'array')
Y10<-(multi_Y0==1)*1
...
for(t in 1:nstage){
  ##generate data set
  tp_expand0<-sapply(1:nsimu, function(r) matrix(rep(trial_process[,r],ninterim[t]),ninterim[t],Ngroup,byrow = T),simplify = 'array')#expand trial_process to a ngroup0 x J x nsimu array
  tp_expand<-sapply(1:nsimu, function(r) rbind(tp_expand[,,r],tp_expand0[,,r]),simplify = 'array')
  Y1<-Y10[1:ngroup_all[t,1],,]*tp_expand
  Y2<-Y20[1:ngroup_all[t,1],,]*tp_expand
...
##save
save(Pr_futility,pr_eff,n,p1_hat_tmp,p2_hat_tmp,file="Rdata/3gcbhml_12.Rdata")
